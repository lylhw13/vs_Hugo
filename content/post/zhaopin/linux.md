...
2. 有名管道(FIFO)
3. 信号(Signal)
4. 信号量(semaphore)：信号量是一个计数器，用于多进程对共享数据的访问，信号量的意图在于进程间同步。
5. 消息(Message)队列：消息队列是存放在内核中的消息链表，每个消息队列由消息队列标识符表示。
6. 共享内存(share memory)
7. 套接字(socket)
...
- 最先匹配法（first fit）
- 下次匹配法（next fit）
    和最先匹配思路类似，只不过会从上次记录的位置开始查找。这种算法查找的结点少，速度快，但是缺点是，较大的空闲分区不容易保留，性能要略逊于最先匹配法
- 最佳匹配法（best fit）
    基本思路是搜索整个链表，将能够装得下该进程的最小空闲分区分配出去。
    缺点：因为要搜索整个链表，速度更慢。
    这种算法性能比最先匹配法和下次匹配法都要差，浪费了更多的内存。原因在于每次选择的都是与进程大小最接近的分区，因此在分割以后剩余的空闲区域将会很小，甚至无法使用， 从而造成浪费。