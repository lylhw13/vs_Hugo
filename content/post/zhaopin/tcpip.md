...
本地地址，本地端口号，外地IP地址，外地端口号

对于 client 端，只有本地端口号可以改变，而且0～1023的端口为系统保留，所以，client端的最大连接数为 65535 - 1024 大致为 64k

对于 server 端，每一个 TCP 连接都要一个文件描述符，所以最大的连接数实际为文件描述符的最大个数。
```sh
ulimit -n # 显示可以打开的最大描述符个数 一般为 1024

ulimit -n 1000000   # 修改可以打开的文件描述符的上限

cat /proc/sys/fs/file-max   # 显示文件系统所支持的最大的文件描述符的个数，是一个很大值，所以一般10k是没有问题
```